...
import (
	...
	"github.com/gaabrielbrocco/go-gateway/internal/dto"
)
...
func (s *AccountService) CreateAccount(input dto.CreateAccountInput) (*dto.AccountOutput, error) {
	...
	// verificando se ja existe o APIKey
...
func (s *AccountService) UpdateBalance(apiKey string, amout float64) (*dto.AccountOutput, error) {
	account, err := s.repository.FindByAPIKey(apiKey)
	if err != nil {
		return nil, err
	}

	account.AddBalance(amout)
